...
public final class ExceptBy {
    ...
    public static <TSource, TKey> IEnumerable<TSource> exceptBy(IEnumerable<TSource> first, IEnumerable<TKey> second, Func1<TSource, TKey> keySelector) {
        return exceptBy(first, second, keySelector, null);
    }

    public static <TSource, TKey> IEnumerable<TSource> exceptBy(IEnumerable<TSource> first, IEnumerable<TKey> second, Func1<TSource, TKey> keySelector, IEqualityComparer<TKey> comparer) {
        if (first == null)
...
final class ExceptByIterator<TSource, TKey> extends AbstractIterator<TSource> {
    ...
    ExceptByIterator(IEnumerable<TSource> first, IEnumerable<TKey> second, Func1<TSource, TKey> keySelector, IEqualityComparer<TKey> comparer) {
        this.first = first;
        this.second = second;
